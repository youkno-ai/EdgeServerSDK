//
// CoEdgeappServerModelBountyColorScheme.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation
#if canImport(AnyCodable)
import AnyCodable
#endif

public struct CoEdgeappServerModelBountyColorScheme: Codable, JSONEncodable, Hashable {

    public enum PatternTypeAsEnum: String, Codable, CaseIterable {
        case _none = "NONE"
        case chessboard = "CHESSBOARD"
        case diagonalLines = "DIAGONAL_LINES"
        case quarterCircles = "QUARTER_CIRCLES"
        case horizontalLines = "HORIZONTAL_LINES"
        case fourDots = "FOUR_DOTS"
        case downwardArrows = "DOWNWARD_ARROWS"
        case unknown = "UNKNOWN"
    }
    public enum DisplayModeAsEnum: String, Codable, CaseIterable {
        case _none = "NONE"
        case random = "RANDOM"
        case ordered = "ORDERED"
        case unknown = "UNKNOWN"
    }
    public var scheme: String?
    public var colors: [CoEdgeappServerModelColor]?
    public var displayMode: String?
    public var patternType: String?
    public var patternTypeAsEnum: PatternTypeAsEnum?
    public var displayModeAsEnum: DisplayModeAsEnum?

    public init(scheme: String? = nil, colors: [CoEdgeappServerModelColor]? = nil, displayMode: String? = nil, patternType: String? = nil, patternTypeAsEnum: PatternTypeAsEnum? = nil, displayModeAsEnum: DisplayModeAsEnum? = nil) {
        self.scheme = scheme
        self.colors = colors
        self.displayMode = displayMode
        self.patternType = patternType
        self.patternTypeAsEnum = patternTypeAsEnum
        self.displayModeAsEnum = displayModeAsEnum
    }

    public enum CodingKeys: String, CodingKey, CaseIterable {
        case scheme
        case colors
        case displayMode
        case patternType
        case patternTypeAsEnum
        case displayModeAsEnum
    }

    // Encodable protocol methods

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encodeIfPresent(scheme, forKey: .scheme)
        try container.encodeIfPresent(colors, forKey: .colors)
        try container.encodeIfPresent(displayMode, forKey: .displayMode)
        try container.encodeIfPresent(patternType, forKey: .patternType)
        try container.encodeIfPresent(patternTypeAsEnum, forKey: .patternTypeAsEnum)
        try container.encodeIfPresent(displayModeAsEnum, forKey: .displayModeAsEnum)
    }
}

