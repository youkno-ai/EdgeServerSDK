//
// BountyClaimSnippet.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation
#if canImport(AnyCodable)
import AnyCodable
#endif

public struct BountyClaimSnippet: Codable, JSONEncodable, Hashable {

    public enum StatusAsEnum: String, Codable, CaseIterable {
        case claimed = "CLAIMED"
        case canceled = "CANCELED"
        case rejected = "REJECTED"
    }
    public var from: User?
    public var claimId: String?
    public var rejection: BountyClaimSnippetRejection?
    public var claimedAt: Int64?
    public var status: String?
    public var statusAsEnum: StatusAsEnum?
    public var rejected: Bool?

    public init(from: User? = nil, claimId: String? = nil, rejection: BountyClaimSnippetRejection? = nil, claimedAt: Int64? = nil, status: String? = nil, statusAsEnum: StatusAsEnum? = nil, rejected: Bool? = nil) {
        self.from = from
        self.claimId = claimId
        self.rejection = rejection
        self.claimedAt = claimedAt
        self.status = status
        self.statusAsEnum = statusAsEnum
        self.rejected = rejected
    }

    public enum CodingKeys: String, CodingKey, CaseIterable {
        case from
        case claimId
        case rejection
        case claimedAt
        case status
        case statusAsEnum
        case rejected
    }

    // Encodable protocol methods

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encodeIfPresent(from, forKey: .from)
        try container.encodeIfPresent(claimId, forKey: .claimId)
        try container.encodeIfPresent(rejection, forKey: .rejection)
        try container.encodeIfPresent(claimedAt, forKey: .claimedAt)
        try container.encodeIfPresent(status, forKey: .status)
        try container.encodeIfPresent(statusAsEnum, forKey: .statusAsEnum)
        try container.encodeIfPresent(rejected, forKey: .rejected)
    }
}

