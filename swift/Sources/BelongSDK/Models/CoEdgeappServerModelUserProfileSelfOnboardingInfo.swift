//
// CoEdgeappServerModelUserProfileSelfOnboardingInfo.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation
#if canImport(AnyCodable)
import AnyCodable
#endif

public struct CoEdgeappServerModelUserProfileSelfOnboardingInfo: Codable, JSONEncodable, Hashable {

    public enum BusinessType: String, Codable, CaseIterable {
        case _none = "NONE"
        case foodBeverage = "FOOD_BEVERAGE"
        case retail = "RETAIL"
        case services = "SERVICES"
        case events = "EVENTS"
        case other = "OTHER"
        case unknown = "UNKNOWN"
    }
    public enum BusinessSubType: String, Codable, CaseIterable {
        case _none = "NONE"
        case coffeeShop = "COFFEE_SHOP"
        case restaurant = "RESTAURANT"
        case bakery = "BAKERY"
        case barPub = "BAR_PUB"
        case retailStore = "RETAIL_STORE"
        case onlineStore = "ONLINE_STORE"
        case freelancer = "FREELANCER"
        case beautySalon = "BEAUTY_SALON"
        case fitnessStudioGym = "FITNESS_STUDIO_GYM"
        case wellnessCenter = "WELLNESS_CENTER"
        case serviceProvider = "SERVICE_PROVIDER"
        case eventOrganizer = "EVENT_ORGANIZER"
        case other = "OTHER"
        case unknown = "UNKNOWN"
    }
    public enum BusinessCurrency: String, Codable, CaseIterable {
        case _none = "NONE"
        case usd = "USD"
        case cad = "CAD"
        case eur = "EUR"
        case ron = "RON"
        case gbp = "GBP"
        case pln = "PLN"
        case gel = "GEL"
        case ars = "ARS"
        case aed = "AED"
        case sar = "SAR"
        case mxn = "MXN"
        case ngn = "NGN"
        case pkr = "PKR"
        case mdl = "MDL"
        case rub = "RUB"
        case uah = "UAH"
        case kzt = "KZT"
        case kes = "KES"
        case ugx = "UGX"
        case uzs = "UZS"
        case xaf = "XAF"
        case egp = "EGP"
        case mwk = "MWK"
        case amd = "AMD"
        case brl = "BRL"
        case gyd = "GYD"
        case huf = "HUF"
        case lbp = "LBP"
        case _try = "TRY"
        case iqd = "IQD"
        case bzd = "BZD"
        case mnt = "MNT"
        case jpy = "JPY"
        case lsl = "LSL"
        case jod = "JOD"
        case mur = "MUR"
        case zar = "ZAR"
        case czk = "CZK"
        case mmk = "MMK"
        case sdg = "SDG"
        case pgk = "PGK"
        case ttd = "TTD"
        case chf = "CHF"
        case gip = "GIP"
        case crc = "CRC"
        case bbd = "BBD"
        case rsd = "RSD"
        case lkr = "LKR"
        case pyg = "PYG"
        case gmd = "GMD"
        case sek = "SEK"
        case byn = "BYN"
        case cve = "CVE"
        case fkp = "FKP"
        case mzn = "MZN"
        case mkd = "MKD"
        case sbd = "SBD"
        case bam = "BAM"
        case ves = "VES"
        case vnd = "VND"
        case etb = "ETB"
        case hnl = "HNL"
        case nad = "NAD"
        case inr = "INR"
        case azn = "AZN"
        case cdf = "CDF"
        case sgd = "SGD"
        case mad = "MAD"
        case pab = "PAB"
        case nio = "NIO"
        case tjs = "TJS"
        case xof = "XOF"
        case mga = "MGA"
        case kwd = "KWD"
        case npr = "NPR"
        case bdt = "BDT"
        case bmd = "BMD"
        case cny = "CNY"
        case ghs = "GHS"
        case dkk = "DKK"
        case awg = "AWG"
        case php = "PHP"
        case ern = "ERN"
        case nok = "NOK"
        case yer = "YER"
        case djf = "DJF"
        case bif = "BIF"
        case bsd = "BSD"
        case fjd = "FJD"
        case isk = "ISK"
        case mru = "MRU"
        case pen = "PEN"
        case myr = "MYR"
        case kpw = "KPW"
        case gtq = "GTQ"
        case vuv = "VUV"
        case lyd = "LYD"
        case kmf = "KMF"
        case jmd = "JMD"
        case xpf = "XPF"
        case xcd = "XCD"
        case srd = "SRD"
        case thb = "THB"
        case zmw = "ZMW"
        case htg = "HTG"
        case qar = "QAR"
        case gnf = "GNF"
        case tnd = "TND"
        case mop = "MOP"
        case scr = "SCR"
        case tzs = "TZS"
        case uyu = "UYU"
        case aoa = "AOA"
        case aud = "AUD"
        case clp = "CLP"
        case omr = "OMR"
        case bhd = "BHD"
        case twd = "TWD"
        case afn = "AFN"
        case bwp = "BWP"
        case lrd = "LRD"
        case svc = "SVC"
        case hrk = "HRK"
        case nzd = "NZD"
        case zwl = "ZWL"
        case irr = "IRR"
        case bob = "BOB"
        case dzd = "DZD"
        case bnd = "BND"
        case ils = "ILS"
        case mvr = "MVR"
        case stn = "STN"
        case sos = "SOS"
        case kyd = "KYD"
        case szl = "SZL"
        case khr = "KHR"
        case ssp = "SSP"
        case bgn = "BGN"
        case shp = "SHP"
        case all = "ALL"
        case csd = "CSD"
        case krw = "KRW"
        case top = "TOP"
        case syp = "SYP"
        case btn = "BTN"
        case cup = "CUP"
        case idr = "IDR"
        case rwf = "RWF"
        case sll = "SLL"
        case ang = "ANG"
        case lak = "LAK"
        case cop = "COP"
        case tmt = "TMT"
        case dop = "DOP"
        case kgs = "KGS"
        case hkd = "HKD"
        case wst = "WST"
        case unknown = "UNKNOWN"
    }
    public enum VoucherDiscountType: String, Codable, CaseIterable {
        case _none = "NONE"
        case flat = "FLAT"
        case percentage = "PERCENTAGE"
        case unknown = "UNKNOWN"
    }
    public enum BountyType: String, Codable, CaseIterable {
        case _none = "NONE"
        case question = "QUESTION"
        case thread = "THREAD"
        case talentSearch = "TALENT_SEARCH"
        case talentRecommendation = "TALENT_RECOMMENDATION"
        case tmobChallenge = "TMOB_CHALLENGE"
        case tmobCoaching = "TMOB_COACHING"
        case order = "ORDER"
        case realestateCustomer = "REALESTATE_CUSTOMER"
        case realestateRecommendation = "REALESTATE_RECOMMENDATION"
        case survey = "SURVEY"
        case classified = "CLASSIFIED"
        case product = "PRODUCT"
        case menu = "MENU"
        case bundle = "BUNDLE"
        case story = "STORY"
        case score = "SCORE"
        case series = "SERIES"
        case checklist = "CHECKLIST"
        case support = "SUPPORT"
        case aiBot = "AI_BOT"
        case deal = "DEAL"
        case promotion = "PROMOTION"
        case autosuggestion = "AUTOSUGGESTION"
        case approval = "APPROVAL"
        case mcq = "MCQ"
        case funding = "FUNDING"
        case auction = "AUCTION"
        case roleplay = "ROLEPLAY"
        case privateListing = "PRIVATE_LISTING"
        case quest = "QUEST"
        case liveStream = "LIVE_STREAM"
        case album = "ALBUM"
        case news = "NEWS"
        case banner = "BANNER"
        case match = "MATCH"
        case contest = "CONTEST"
        case trivia = "TRIVIA"
        case lottery = "LOTTERY"
        case badgeCollection = "BADGE_COLLECTION"
        case raffle = "RAFFLE"
        case calendarEvent = "CALENDAR_EVENT"
        case currency = "CURRENCY"
        case reservation = "RESERVATION"
        case quickClaim = "QUICK_CLAIM"
        case campaign = "CAMPAIGN"
        case expressCampaign = "EXPRESS_CAMPAIGN"
        case geoLocation = "GEO_LOCATION"
        case feedback = "FEEDBACK"
        case unknown = "UNKNOWN"
    }
    public enum BountyAttachmentType: String, Codable, CaseIterable {
        case image = "IMAGE"
        case gif = "GIF"
        case tiff = "TIFF"
        case video = "VIDEO"
        case audio = "AUDIO"
        case lottie = "LOTTIE"
        case pdf = "PDF"
        case doc = "DOC"
        case youtube = "YOUTUBE"
        case website = "WEBSITE"
        case file = "FILE"
        case other = "OTHER"
        case txt = "TXT"
        case csv = "CSV"
        case eml = "EML"
        case ics = "ICS"
        case msg = "MSG"
        case notFile = "NOT_FILE"
    }
    public enum BountyVisibility: String, Codable, CaseIterable {
        case global = "GLOBAL"
        case _internal = "INTERNAL"
        case userInternal = "USER_INTERNAL"
        case byLink = "BY_LINK"
    }
    public var businessName: String?
    public var businessType: BusinessType?
    public var businessSubType: BusinessSubType?
    public var businessLogoUrl: String?
    public var businessBio: String?
    public var businessWebsiteUrl: String?
    public var businessCurrency: BusinessCurrency?
    public var useOwnCategories: Bool?
    public var voucherCode: String?
    public var voucherDiscountType: VoucherDiscountType?
    public var voucherTitle: String?
    public var voucherPosDiscountCode: String?
    public var voucherDescription: String?
    public var voucherFlatDiscount: CoEdgeappServerModelReward?
    public var voucherPercentageDiscount: Double?
    public var voucherMaxQty: Int?
    public var voucherImageUrl: String?
    public var voucherExpiresAt: Int64?
    public var voucherLifespanMs: Int64?
    public var bountyType: BountyType?
    public var bountyImageUrl: String?
    public var bountyAttachmentType: BountyAttachmentType?
    public var bountyVisibility: BountyVisibility?
    public var bountyMaxRecipientsQty: Int?
    public var companyId: String?
    public var voucherDefId: String?
    public var bountyId: String?
    public var completedAt: Int64?
    public var valid: Bool?
    public var completed: Bool?

    public init(businessName: String? = nil, businessType: BusinessType? = nil, businessSubType: BusinessSubType? = nil, businessLogoUrl: String? = nil, businessBio: String? = nil, businessWebsiteUrl: String? = nil, businessCurrency: BusinessCurrency? = nil, useOwnCategories: Bool? = nil, voucherCode: String? = nil, voucherDiscountType: VoucherDiscountType? = nil, voucherTitle: String? = nil, voucherPosDiscountCode: String? = nil, voucherDescription: String? = nil, voucherFlatDiscount: CoEdgeappServerModelReward? = nil, voucherPercentageDiscount: Double? = nil, voucherMaxQty: Int? = nil, voucherImageUrl: String? = nil, voucherExpiresAt: Int64? = nil, voucherLifespanMs: Int64? = nil, bountyType: BountyType? = nil, bountyImageUrl: String? = nil, bountyAttachmentType: BountyAttachmentType? = nil, bountyVisibility: BountyVisibility? = nil, bountyMaxRecipientsQty: Int? = nil, companyId: String? = nil, voucherDefId: String? = nil, bountyId: String? = nil, completedAt: Int64? = nil, valid: Bool? = nil, completed: Bool? = nil) {
        self.businessName = businessName
        self.businessType = businessType
        self.businessSubType = businessSubType
        self.businessLogoUrl = businessLogoUrl
        self.businessBio = businessBio
        self.businessWebsiteUrl = businessWebsiteUrl
        self.businessCurrency = businessCurrency
        self.useOwnCategories = useOwnCategories
        self.voucherCode = voucherCode
        self.voucherDiscountType = voucherDiscountType
        self.voucherTitle = voucherTitle
        self.voucherPosDiscountCode = voucherPosDiscountCode
        self.voucherDescription = voucherDescription
        self.voucherFlatDiscount = voucherFlatDiscount
        self.voucherPercentageDiscount = voucherPercentageDiscount
        self.voucherMaxQty = voucherMaxQty
        self.voucherImageUrl = voucherImageUrl
        self.voucherExpiresAt = voucherExpiresAt
        self.voucherLifespanMs = voucherLifespanMs
        self.bountyType = bountyType
        self.bountyImageUrl = bountyImageUrl
        self.bountyAttachmentType = bountyAttachmentType
        self.bountyVisibility = bountyVisibility
        self.bountyMaxRecipientsQty = bountyMaxRecipientsQty
        self.companyId = companyId
        self.voucherDefId = voucherDefId
        self.bountyId = bountyId
        self.completedAt = completedAt
        self.valid = valid
        self.completed = completed
    }

    public enum CodingKeys: String, CodingKey, CaseIterable {
        case businessName
        case businessType
        case businessSubType
        case businessLogoUrl
        case businessBio
        case businessWebsiteUrl
        case businessCurrency
        case useOwnCategories
        case voucherCode
        case voucherDiscountType
        case voucherTitle
        case voucherPosDiscountCode
        case voucherDescription
        case voucherFlatDiscount
        case voucherPercentageDiscount
        case voucherMaxQty
        case voucherImageUrl
        case voucherExpiresAt
        case voucherLifespanMs
        case bountyType
        case bountyImageUrl
        case bountyAttachmentType
        case bountyVisibility
        case bountyMaxRecipientsQty
        case companyId
        case voucherDefId
        case bountyId
        case completedAt
        case valid
        case completed
    }

    // Encodable protocol methods

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encodeIfPresent(businessName, forKey: .businessName)
        try container.encodeIfPresent(businessType, forKey: .businessType)
        try container.encodeIfPresent(businessSubType, forKey: .businessSubType)
        try container.encodeIfPresent(businessLogoUrl, forKey: .businessLogoUrl)
        try container.encodeIfPresent(businessBio, forKey: .businessBio)
        try container.encodeIfPresent(businessWebsiteUrl, forKey: .businessWebsiteUrl)
        try container.encodeIfPresent(businessCurrency, forKey: .businessCurrency)
        try container.encodeIfPresent(useOwnCategories, forKey: .useOwnCategories)
        try container.encodeIfPresent(voucherCode, forKey: .voucherCode)
        try container.encodeIfPresent(voucherDiscountType, forKey: .voucherDiscountType)
        try container.encodeIfPresent(voucherTitle, forKey: .voucherTitle)
        try container.encodeIfPresent(voucherPosDiscountCode, forKey: .voucherPosDiscountCode)
        try container.encodeIfPresent(voucherDescription, forKey: .voucherDescription)
        try container.encodeIfPresent(voucherFlatDiscount, forKey: .voucherFlatDiscount)
        try container.encodeIfPresent(voucherPercentageDiscount, forKey: .voucherPercentageDiscount)
        try container.encodeIfPresent(voucherMaxQty, forKey: .voucherMaxQty)
        try container.encodeIfPresent(voucherImageUrl, forKey: .voucherImageUrl)
        try container.encodeIfPresent(voucherExpiresAt, forKey: .voucherExpiresAt)
        try container.encodeIfPresent(voucherLifespanMs, forKey: .voucherLifespanMs)
        try container.encodeIfPresent(bountyType, forKey: .bountyType)
        try container.encodeIfPresent(bountyImageUrl, forKey: .bountyImageUrl)
        try container.encodeIfPresent(bountyAttachmentType, forKey: .bountyAttachmentType)
        try container.encodeIfPresent(bountyVisibility, forKey: .bountyVisibility)
        try container.encodeIfPresent(bountyMaxRecipientsQty, forKey: .bountyMaxRecipientsQty)
        try container.encodeIfPresent(companyId, forKey: .companyId)
        try container.encodeIfPresent(voucherDefId, forKey: .voucherDefId)
        try container.encodeIfPresent(bountyId, forKey: .bountyId)
        try container.encodeIfPresent(completedAt, forKey: .completedAt)
        try container.encodeIfPresent(valid, forKey: .valid)
        try container.encodeIfPresent(completed, forKey: .completed)
    }
}

