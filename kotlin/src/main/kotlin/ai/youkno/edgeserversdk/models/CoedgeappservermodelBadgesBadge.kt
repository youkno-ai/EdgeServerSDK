/**
 *
 * Please note:
 * This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * Do not edit this file manually.
 *
 */

@file:Suppress(
    "ArrayInDataClass",
    "EnumEntryName",
    "RemoveRedundantQualifierName",
    "UnusedImport"
)

package ai.youkno.edgeserversdk.models

import ai.youkno.edgeserversdk.models.CoedgeappservermodelBadgesBadgeInfo
import ai.youkno.edgeserversdk.models.CoedgeappservermodelReward

import com.squareup.moshi.Json
import com.squareup.moshi.JsonClass

/**
 * 
 *
 * @param id 
 * @param info 
 * @param awardedAt 
 * @param negative 
 * @param reward 
 * @param virtual 
 * @param type 
 * @param label 
 * @param valid 
 * @param description 
 * @param code 
 * @param negativeValue 
 * @param image 
 * @param icon 
 */


data class CoedgeappservermodelBadgesBadge (

    @Json(name = "id")
    val id: kotlin.String? = null,

    @Json(name = "info")
    val info: CoedgeappservermodelBadgesBadgeInfo? = null,

    @Json(name = "awardedAt")
    val awardedAt: kotlin.Long? = null,

    @Json(name = "negative")
    val negative: kotlin.Boolean? = null,

    @Json(name = "reward")
    val reward: CoedgeappservermodelReward? = null,

    @Json(name = "virtual")
    val virtual: kotlin.Boolean? = null,

    @Json(name = "type")
    val type: CoedgeappservermodelBadgesBadge.Type? = null,

    @Json(name = "label")
    val label: kotlin.String? = null,

    @Json(name = "valid")
    val valid: kotlin.Boolean? = null,

    @Json(name = "description")
    val description: kotlin.String? = null,

    @Json(name = "code")
    val code: kotlin.String? = null,

    @Json(name = "negativeValue")
    val negativeValue: kotlin.Boolean? = null,

    @Json(name = "image")
    val image: kotlin.String? = null,

    @Json(name = "icon")
    val icon: kotlin.String? = null

) {

    /**
     * 
     *
     * Values: NONE,OFFICIAL,REQUEST_OFFICIAL_RESPONSE,CUSTOM
     */
    @JsonClass(generateAdapter = false)
    enum class Type(val value: kotlin.String) {
        @Json(name = "NONE") NONE("NONE"),
        @Json(name = "OFFICIAL") OFFICIAL("OFFICIAL"),
        @Json(name = "REQUEST_OFFICIAL_RESPONSE") REQUEST_OFFICIAL_RESPONSE("REQUEST_OFFICIAL_RESPONSE"),
        @Json(name = "CUSTOM") CUSTOM("CUSTOM");
    }

}

